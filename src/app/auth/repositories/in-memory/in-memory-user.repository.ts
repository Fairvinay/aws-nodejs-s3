...
export class InMemoryUserRepository implements UserRepository {
  ...
  getUserByEmail(email: string): Promise<User> {
    ...
    this.readUserFromJSONfile(email);
    return new Promise((resolve, reject) => {
      user ? resolve(user) : reject("Invalid Email");
    });
  }
  ...
  resyncUSERSandRegisteredUser() { 
   ...
   let uniqUsers = 	usersRegistered.reduce(function(arr:Array<User>, curr:User) {
			let exits =   arr.filter( elem => checkEmail(elem, curr.email ));
			if( exits.length ==0){
